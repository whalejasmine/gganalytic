...
TRN_PRED_FEATS='../output/lgb_predfts_trn_prd_feats.npy'
TES_PRED_FEATS='../output/lgb_predfts_tes_prd_feats.npy'
...
class xgbl(object):
    ...
    def cv(self, nfolds=5, submission=True):
        ...
        if submission:
            ...
            self.x_test[['PredictedLogRevenue']].to_csv('../output/{}.csv'.format(self.name), index=True)
...
if __name__ == "__main__":
    ...
    m = xgbl(name='xgbl_m2',remove_columns = excluded_features, comment='xgboost model using lgm pred features',predict_feats=True,debug=False)
