...
class lm(object):
    ...
    def lm_evaluate(self,**Tunparams):
        warnings.filterwarnings("ignore")
    ...
    def cv(self,X=None, nfolds=5, submission=True, tune=True, baseline=None):
        ...
        #Tune parameters
        if tune:
                bo = BayesianOptimization(self.lm_evaluate, self.Tunparams)
    ...
    def feature_selection_eval(self, nfolds, set=2, file='../output/log/log_lm_fw.txt',  nskip=0):
...
if __name__ == "__main__":
    ...
    m = lm(name='lm_m',remove_columns = excluded_features, comment='LM with cat',predict_feats=True,debug=False,lm_seed=17,
                    BASE_X_PATH='../output/feats/df_feats_cats.f', TRN_PRED_FEATS='../output/feats/xgbl_predfts_2_trn_prd_feats.npy',
                    TES_PRED_FEATS='../output/feats/xgbl_predfts_2_tes_prd_feats.npy',clf=Ridge
                    ,param={'normalize': True})
    full_rmse, _, _,_= m.cv(tune=True)
