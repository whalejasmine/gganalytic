...
class lm(object):
    ...
    def cv(self, nfolds=5, submission=True):
        ...
        return full_rmse, oof_preds, preds



if __name__ == "__main__":
    predfts = lm(name='lm_feats',remove_columns = excluded_features, clf=Ridge,comment='lm feats',predict_feats=False, debug=True, BASE_X_PATH = '../output/feats/df_feats_cats.f')
    full_rmse, oof_preds, preds_test = predfts.cv(submission=False)
