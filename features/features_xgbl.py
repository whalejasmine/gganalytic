...
class xgbl(object):
    ...
    def cv(self, nfolds=5, submission=True):
        ...
        # for stack
        np.save('../output/{}_trn_prd_feats'.format(self.name),oof_preds)
        np.save('../output/{}_tes_prd_feats'.format(self.name),preds)
        return self.feature_importance_df, full_rmse, oof_preds, preds



if __name__ == "__main__":
    predfts = xgbl(name='xgbl_predfts',remove_columns = excluded_features, comment='xgbl_predfts',predict_feats=False, debug=False)
