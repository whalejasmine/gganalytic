...
def multi_apply_func_on_series(df=None, func=None, n_jobs=1):
    p = Pool(n_jobs)
...
def main(nrows=None):
    # Convert train
    zptn = zipfile.ZipFile('../input/train_v2.csv.zip') 
    train = pd.read_csv(zptn.open('train_v2.csv'), dtype='object', nrows=nrows, encoding='utf-8')
    train = develop_json_fields(df=train)
    logger.info('Train done')

    # Convert test
    zpte = zipfile.ZipFile('../input/test_v2.csv.zip') 
    test = pd.read_csv(zpte.open('test_v2.csv'), dtype='object', nrows=nrows, encoding='utf-8')
